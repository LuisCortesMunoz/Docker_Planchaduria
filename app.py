...
# app.py
# Backend completo:
# - Auth con código (registro y reset de contraseña)
# - Pedidos cliente y admin
# - Worker / HMI
# - NUEVO: Historial, pendientes y resumen diario
# ================================
...
# =========================================================
ESTADOS_PENDIENTES  = {"pendiente", "en_proceso", "en proceso", "planchado", ""}
ESTADOS_FINALIZADOS = {"listo", "completado", "entregado"}
...
def es_pendiente(estado: str) -> bool:
    return normalizar_estado(estado) in ESTADOS_PENDIENTES

def es_finalizado(estado: str) -> bool:
    return normalizar_estado(estado) in ESTADOS_FINALIZADOS

def calcular_dias_espera(fecha_ingreso: str) -> int:
    """Días transcurridos desde fechaIngreso hasta hoy."""
    try:
...
def enriquecer_pedido(p: dict) -> dict:
    """Agrega _pendiente, _finalizado y _dias_espera a un dict de pedido."""
    estado_norm      = normalizar_estado(p.get("Estado"))
    p["_pendiente"]  = es_pendiente(estado_norm)
    p["_finalizado"] = es_finalizado(estado_norm)
    p["_dias_espera"] = calcular_dias_espera(p.get("fechaIngreso", ""))
    return p
...
def firebase_sign_in(email, password):
    ...
    resp = requests.post(url, json={"email": email, "password": password, "returnSecureToken": True}, timeout=30)
    data = resp.json()
...
def email_exists_in_firebase(email):
    try:
        firebase_auth.get_user_by_email(normalize_email(email))
        return True
...
def send_register_code_email(to_email, nombre, code):
    ...
    <div style="max-width:520px; margin:0 auto; padding:24px; border:1px solid #eee; border-radius:16px;">
      ...
      <div style="font-size:32px; font-weight:700; letter-spacing:6px; text-align:center; margin:24px 0; color:#111;">{code}</div>
...
def generate_folio():
    ...
    return "#" + str(contador).zfill(5), contador
...
def home():
    return jsonify({"ok": True, "message": "Backend Render activo",
...
def api_verify_register_code():
    ...
    try:
        ...
        user = {
            ...
            "email": email, "telefono": telefono, "isAdmin": is_admin_uid(uid)
        }
        doc_ref.delete()
        return jsonify({"ok": True, "message": "Cuenta creada correctamente.",
                        "token": auth_data.get("idToken"),
                        "refreshToken": auth_data.get("refreshToken"), "user": user}), 200
    except Exception as e:
...
def api_resend_register_code():
    ...
    try:
        ...
        now        = now_mx()
        expires_at = now + timedelta(minutes=REGISTER_CODE_MINUTES)
        doc_ref.update({"code": code, "attempts": 0, "verified": False,
...
def api_login():
    ...
    try:
        ...
        return jsonify({"ok": True, "message": "Inicio de sesión correcto",
                        "token": auth_data.get("idToken"),
                        "refreshToken": auth_data.get("refreshToken"), "user": user}), 200
    except Exception as e:
...
def api_me():
    ...
    user = {
        ...
        "telefono": profile.get("telefono", ""), "isAdmin": request.user_is_admin
    }
...
def api_create_order_client():
    ...
    payload = {
        ...
        "tipoPrenda": tipo_prenda, "material": material,
        "cantidad": cantidad, "precio": None,
        ...
        "fotos": [], "rutina_activa": False,
        "activado_hmi": False, "hmi_activated_at": "",
        "started_at": "", "completed_at": "",
        "ultimo_error": "", "created_at": ahora, "updated_at": ahora
    }
...
def api_admin_orders_create():
    ...
    payload = {
        ...
        "fechaIngreso": fecha_ingreso, "FechaEntrega": fecha_entrega,
        "notas": notas, "Estado": "pendiente", "Validado": False,
        "origenCliente": False, "FolioIngresado": folio,
        "fotos": [], "rutina_activa": False,
        "activado_hmi": False, "hmi_activated_at": "",
        "started_at": "", "completed_at": "",
        "ultimo_error": "", "created_at": ahora, "updated_at": ahora
...
def api_worker_activate_by_folio(folio):
    ...
    payload = {"activado_hmi": True, "hmi_activated_at": now_mx().isoformat(),
    ...
    return jsonify({"ok": True, "message": f"Pedido activado correctamente: {folio}",
                    "order": order_doc_to_json(doc_ref.get())}), 200
...
def api_worker_active_order():
    ...
    if not snap.exists:
        ...
        return jsonify({"ok": True, "order": None}), 200
    return jsonify({"ok": True, "order": order_doc_to_json(snap),
                    "current_order_id": current_order_id,
                    "current_order_folio": current_order_folio}), 200

@app.route("/api/worker/next-order", methods=["GET"])
def api_worker_next_order():
    docs  = (fs.collection("pedidos")
               .where("Estado", "==", "pendiente")
               .where("activado_hmi", "==", True)
               .stream())
    items = []
    for doc in docs:
        item = order_doc_to_json(doc)
        if was_created_after_backend_start(item.get("created_at", "")):
            items.append(item)
...
def api_worker_order_start(order_id):
    ...
    if not doc_ref.get().exists:
        return fail("Pedido no encontrado", 404)
    doc_ref.update({"Estado": "en_proceso", "rutina_activa": True,
...
def api_worker_order_status(order_id):
    ...
    if estado == "en_proceso": payload.update({"rutina_activa": True,  "started_at":   now_mx().isoformat()})
    if estado == "planchado":  payload["rutina_activa"] = True
    if estado == "listo":      payload.update({"rutina_activa": False, "completed_at": now_mx().isoformat()})
    if estado == "entregado":  payload.update({"rutina_activa": False, "Validado": True})
    if estado in {"pendiente", "en_proceso", "planchado", "listo"}:
        payload["Validado"] = False
    doc_ref.update(payload)
    order = order_doc_to_json(doc_ref.get())
    if estado in {"listo", "entregado"}:
        guardar_estado(activo=(estado != "entregado"),
...
def api_worker_order_photo(order_id):
    ...
    try:
        ...
        snap  = doc_ref.get()
        fotos = (snap.to_dict() or {}).get("fotos", [])
        if not isinstance(fotos, list):
            fotos = []
...
def api_worker_order_complete(order_id):
    ...
    if not doc_ref.get().exists:
        return fail("Pedido no encontrado", 404)
    doc_ref.update({"Estado": "listo", "rutina_activa": False,
...
def api_worker_order_error(order_id):
    ...
    doc_ref.update({"Estado": "pendiente", "rutina_activa": False,
...
def api_pedidos_historico():
    """
    Pedidos filtrados por fecha y/o estado.

    Query params:
      fecha_desde  YYYY-MM-DD  (ej: 2026-04-01)
      fecha_hasta  YYYY-MM-DD  (ej: 2026-04-30)
      estado       pendiente | en_proceso | planchado | listo | entregado | todos
      solo_hoy     true/false
      limite       int (default 200, max 500)
    """
    try:
        solo_hoy     = request.args.get("solo_hoy",    "false").lower() == "true"
        fecha_desde  = request.args.get("fecha_desde", "").strip()
        fecha_hasta  = request.args.get("fecha_hasta", "").strip()
        estado_param = request.args.get("estado",      "todos").strip().lower()
        ...
        # ── Consulta Firestore ──────────────────────────────────────────────
        ref = fs.collection("pedidos")

        if estado_param != "todos":
            variantes = {
                "pendiente":  ["pendiente"],
            ...
            estados_buscar = variantes.get(estado_param, [estado_param])
            if len(estados_buscar) == 1:
                ref = ref.where("Estado", "==", estados_buscar[0])
            else:
                ref = ref.where("Estado", "in", estados_buscar)

        docs    = list(ref.order_by("fechaIngreso",
                                    direction=firestore.Query.DESCENDING)
                          .limit(limite)
                          .stream())
        pedidos = [order_doc_to_json(d) for d in docs]

        # ── Filtro de fechas en Python ───────────────────────────────────────
        if fecha_desde:
            pedidos = [p for p in pedidos if str(p.get("fechaIngreso", "")) >= fecha_desde]
        if fecha_hasta:
            pedidos = [p for p in pedidos if str(p.get("fechaIngreso", "")) <= fecha_hasta]
        ...
        resumen = {
            "total":       len(pedidos),
            "pendientes":  sum(1 for p in pedidos if p["_pendiente"]),
            "finalizados": sum(1 for p in pedidos if p["_finalizado"]),
            "en_proceso":  sum(1 for p in pedidos
                               if normalizar_estado(p.get("Estado"))
                               in ("en_proceso", "en proceso", "planchado")),
        }

        return jsonify({
            "ok": True, "resumen": resumen, "pedidos": pedidos,
            "filtros": {"fecha_desde": fecha_desde, "fecha_hasta": fecha_hasta,
                        "estado": estado_param, "solo_hoy": solo_hoy},
        }), 200

    except Exception as e:
        return fail(f"Error al obtener historial: {e}", 500)


@app.route("/api/pedidos/pendientes", methods=["GET"])
def api_pedidos_pendientes():
    """
    Pedidos sin finalizar en los últimos N días,
    ...
    try:
        ...
        fecha_corte = (now_mx() - timedelta(days=dias_atras)).strftime("%Y-%m-%d")

        docs = (
            fs.collection("pedidos")
            .where("Estado", "not-in", list(ESTADOS_FINALIZADOS))
            .order_by("Estado")         # requerido por Firestore con not-in
        ...
        pedidos = []
        for d in docs:
            p = order_doc_to_json(d)
            if str(p.get("fechaIngreso", "")) >= fecha_corte:
                p["_pendiente"]   = True
                p["_finalizado"]  = False
                p["_dias_espera"] = calcular_dias_espera(p.get("fechaIngreso", ""))
                pedidos.append(p)

        pedidos.sort(key=lambda x: x.get("fechaIngreso", ""))
        rezagados = [p for p in pedidos if p["_dias_espera"] > 2]

        return jsonify({
            "ok": True, "total": len(pedidos),
            "rezagados": len(rezagados),
            "dias_atras": dias_atras, "pedidos": pedidos,
        }), 200

    except Exception as e:
        return fail(f"Error al obtener pendientes: {e}", 500)


@app.route("/api/pedidos/resumen-diario", methods=["GET"])
def api_pedidos_resumen_diario():
    """
    Conteo por día y estado. Útil para el dashboard.
    ...
    try:
        ...
        fecha_corte = (now_mx() - timedelta(days=dias)).strftime("%Y-%m-%d")

        docs = fs.collection("pedidos").where("fechaIngreso", ">=", fecha_corte).stream()

        por_fecha: dict = {}
        for d in docs:
            p     = order_doc_to_json(d)
            fecha = str(p.get("fechaIngreso", "sin_fecha"))
            ...
            if fecha not in por_fecha:
                por_fecha[fecha] = {
                    ...
                    "listo": 0, "entregado": 0, "otros": 0,
                }

            por_fecha[fecha]["total"] += 1
            if   est in ("pendiente", ""):
                por_fecha[fecha]["pendiente"]  += 1
            elif est in ("en_proceso", "en proceso", "planchado"):
                por_fecha[fecha]["en_proceso"] += 1
            elif est == "listo":
                por_fecha[fecha]["listo"]      += 1
            elif est == "entregado":
                por_fecha[fecha]["entregado"]  += 1
            else:
                por_fecha[fecha]["otros"]      += 1

        resultado = sorted(por_fecha.values(), key=lambda x: x["fecha"], reverse=True)
        return jsonify({"ok": True, "dias": dias, "resumen": resultado}), 200

    except Exception as e:
        return fail(f"Error al obtener resumen diario: {e}", 500)
...
def subir_foto():
    ...
    try:
        ...
        blob.make_public()
        foto_info = {
            "usuario": usuario, "nombre": nombre_final,
            "url": blob.public_url, "content_type": content_type,
            "fecha": fecha, "hora": hora,
